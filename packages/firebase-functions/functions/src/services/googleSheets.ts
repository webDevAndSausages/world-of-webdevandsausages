...
const functionsOauthClient = new OAuth2Client(config.GOOGLE.ID, config.GOOGLE.SECRET, GOOGLE_AUTH_REDIRECT)
...
function getAuthorizedClient() {
	return Future((rej, res) => {
		...
		if (!res) {
			return tryP(() => tokensRef.doc('googleSheetsApi').get()).map(docDataOrNull).chain((data) => {
				oauthTokens = data

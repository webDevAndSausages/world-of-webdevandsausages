...
import { areValidResults, docDataOrNull, findByEmailAndPassword, findIndexOfRegistration, formatDate } from '../utils'
import { sendMail } from '../services/mail'
import { participantSchema } from './schemas'
import { eventsRef } from '../services/db'
import { allPass, compose, prop, merge, identity, uniq, concat, find, propEq, or, propOr, inc } from 'ramda'
...
const isNotClosed = compose((date) => moment().isSameOrBefore(date), prop('registrationCloses'))

const isOpen = compose((date) => moment().isSameOrAfter(date), prop('registrationOpens'))
...
const pushToEventQueue = (email: string, type: registrationQueue, eventId: string, details: object) => {
	...
	return tryP(() => eventsRef.doc(eventId).update({ [type]: updatedList })).bimap(
		identity,
		merge({
...
const findInRegistrationQueue = (eventId: string, email: string, token: string) => (details) => {
	const valid = areValidResults(details)
	if (valid) {
		const isRegistered = compose(findByEmailAndPassword(email, token), prop('registered'))(details)
		const isWaitListed = compose(findByEmailAndPassword(email, token), prop('waitListed'))(details)

		if (!isRegistered && !isWaitListed) {
			return reject(createError(401, 'Your email and verificationToken do not match a registration.'))
		}
		...
		if (isWaitListed) {
			const place = compose(inc, findIndexOfRegistration(isWaitListed), prop('waitListed'))(details)
...
export const verifyRegistration = (request: Request, response: Response, next: NextFunction) => {
	const eventId = request.params.eventId

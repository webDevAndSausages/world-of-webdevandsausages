...
import { filter, traverse, compose, prop, evolve, length, always, ifElse, has } from 'ramda'
...
const isFutureEvent = compose((date) => moment().isSameOrBefore(moment(date).add(26, 'hours')), prop('datetime'))
...
const transformForPublicApi = (current: IEvent[]): any => {
	...
	const feedbackOpen = compose(ifElse(has('datetime'), compose(isWithin24Hours, prop('datetime')), always(false)))(
		currentEvent
	)
...
export const createEvent = (request: Request, respose: Response, next: NextFunction) => {
...
export const updateEvent = (request: Request, respose: Response, next: NextFunction) => {
...
export const removeEvent = (request: Request, response: Response, next: NextFunction) => {
	const eventId = request.params.id

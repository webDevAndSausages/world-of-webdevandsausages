<script>
	...
	const onEscape = (e) => {
		if (e.key.includes('Esc') && ($stateMachine === 'active' || $stateMachine === 'warning')) {
			$form.email = '';
	...
	$: inputClasses = cc([{}, 'outline-none text-base px-2 pb-2 pt-2 w-full border bg-transparent']);
	...
	$: $form.error = validateEmail($form.email) ? validateEmail($form.email) : null;
	...
	$: show =
		$stateMachine === 'active' || $stateMachine === 'warning' || $stateMachine === 'loading';
</script>

<svelte:window on:keydown={onEscape} />
{#if show}
	<div class="relative" transition:fly={{ duration: 100, y: 100, opacity: 0.0, easing: linear }}>
...
<style>
	...
	input.active {
		...
		border-image: linear-gradient(45deg, var(--brand-secondary) 0%, var(--brand-primary) 100%);
		border-image-slice: 1 1 1 1;
	}

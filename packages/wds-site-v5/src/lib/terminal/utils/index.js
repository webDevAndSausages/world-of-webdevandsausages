...
const formatWithZone = (zonedDate) => format(zonedDate, readablePattern, { timeZone });
...
export const showForStatusOf = (...statuses) =>
	compose((s) => includes(s, statuses), propOr({}, 'status'));
...
export const isValidToken = createValidator(both(includes('-'), minLen(5)), 'Invalid token format');
